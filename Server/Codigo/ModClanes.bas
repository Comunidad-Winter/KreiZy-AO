...
If Not UserList(UserIndex).Stats.UserSkills(Liderazgo) = 100 Then
    Call SendData(ToIndex, UserIndex, 0, "||¡¡Debes tener 100 en liderazgo!!" & FONTTYPE_INFO)
    Exit Function
End If

If UserList(UserIndex).Stats.GLD < 5000000 Then
    Call SendData(ToIndex, UserIndex, 0, "||¡¡Debes tener 5kk para fundar clan!!" & FONTTYPE_INFO)
    Exit Function
End If
 
 If Not UserList(UserIndex).flags.Canje >= 1 Then
    Call SendData(ToIndex, UserIndex, 0, "||¡¡Debes tener 1 punto de canjeo!!" & FONTTYPE_INFO)
    Exit Function
End If

 If UserList(UserIndex).flags.Honor < 750 Then
      Call SendData(ToIndex, UserIndex, 0, "||¡Necesitas 750 de honor!" & FONTTYPE_VIP)
      Exit Function
      End If
      
           If Not UserList(UserIndex).Faccion.Matados(Neutral) And UserList(UserIndex).Faccion.Matados(Caos) And UserList(UserIndex).Faccion.Matados(Real) = 300 Then
            Call SendData(ToIndex, UserIndex, 0, "||Debes tener 300 matados." & FONTTYPE_INFO)
 Exit Function
 End If
...
End If
   If Not TieneObjetos(407, 1, UserIndex) Then
                ...
                Call SendData(ToIndex, UserIndex, 0, "||¡¡Debes tener la gema de clan para fundar clan!!" & FONTTYPE_INFO)
 Exit Function
 End If
CanCreateGuild = True
...
End If
  ...
  Call QuitarObjetos(407, 1, Index)
   UserList(Index).Stats.GLD = UserList(Index).Stats.GLD - 5000000
 UserList(Index).flags.Honor = UserList(Index).flags.Honor - 750
UserList(Index).flags.Canje = UserList(Index).flags.Canje - 1
