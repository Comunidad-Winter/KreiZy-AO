...
If UserList(UserIndex).Clase = MINERO Then
    MiObj.Amount = Fix(288 + ((0.31 + 0.08 * Buleano(UserList(UserIndex).Recompensas(1) = 1 And UserList(UserIndex).Invent.HerramientaEqpObjIndex = PICO_EXPERTO)) * UserList(UserIndex).Stats.UserSkills(Mineria)))
Else: MiObj.Amount = 15
End If

If Not MeterItemEnInventario(UserIndex, MiObj) Then Call TirarItemAlPiso(UserList(UserIndex).POS, MiObj)
Call SubirSkill(UserIndex, Mineria, 1900)
