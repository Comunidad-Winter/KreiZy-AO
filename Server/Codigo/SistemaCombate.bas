...
If Npclist(NpcIndex).Stats.MinHP <= 0 Then
   If Npclist(NpcIndex).Name = "Rey del Castillo" Then Npclist(NpcIndex).Stats.MinHP = Npclist(NpcIndex).Stats.MaxHP
End If
...
Else: Exit Sub
End If
If Npclist(Victima).POS.Map = Mapa_Castillo And Npclist(Victima).Name = "Rey del Castillo" And Npclist(Victima).Stats.MinHP > 7000 And Npclist(Victima).Stats.MinHP <> 19000 Then Call SendData(ToAll, 0, 0, "El rey del Castillo está siendo atacado!" & FONTTYPE_FENIX)
If Npclist(Victima).POS.Map = Mapa_Castillo And Npclist(Victima).Name = "Rey del Castillo" And Npclist(Victima).Stats.MinHP > 10 And Npclist(Victima).Stats.MinHP < 6000 Then Call SendData(ToAll, 0, 0, "El rey del Castillo está apunto de morir!" & FONTTYPE_FENIX)
...
If Npclist(NpcIndex).Name = "Rey del Castillo" Then
    If (Npclist(NpcIndex).POS.Map = Mapa_Castillo) Then
        Dim Castiact As String
        If Npclist(NpcIndex).POS.Map = Mapa_Castillo Then Castiact = Castillo
...
'   Exit Sub
'End If
If UserList(AtacanteIndex).flags.GrupoAzul = True And UserList(VictimaIndex).flags.GrupoAzul = True Then
Call SendData(ToIndex, AtacanteIndex, 0, "||No puedes atacar a un usurio de tu mismo grupo de batalla!" & FONTTYPE_TALK)
Exit Sub
End If
If UserList(AtacanteIndex).flags.GrupoRojo = True And UserList(VictimaIndex).flags.GrupoRojo = True Then
Call SendData(ToIndex, AtacanteIndex, 0, "||No puedes atacar a un usurio de tu mismo grupo de batalla!" & FONTTYPE_TALK)
Exit Sub
End If
...
If UserList(VictimaIndex).Stats.MinHP <= 0 Then
     Call ContarMuerte(VictimaIndex, AtacanteIndex)
        If UserList(AtacanteIndex).flags.Honor < UserList(VictimaIndex).flags.Honor Then
        UserList(AtacanteIndex).flags.Honor = UserList(AtacanteIndex).flags.Honor + 20
       Call SendData(ToIndex, AtacanteIndex, 0, "||Ganastes 20 puntos de honor al matar a un usuario superior a ti!" & FONTTYPE_TALK)
   Else
        UserList(AtacanteIndex).flags.Honor = UserList(AtacanteIndex).flags.Honor + 10
     Call SendData(ToIndex, AtacanteIndex, 0, "||Ganastes 10 puntos de honor al matar a un usuario menor a ti!" & FONTTYPE_TALK)
   End If
...
If UserList(AttackerIndex).POS.Map = Mapa_Castillo Then Exit Sub
