       ...
       If Len(UserList(TempCharIndex).GuildInfo.GuildName) > 0 Then
        ...
        Else
         'Sistema de Honor - Peto.
            ...
            End If
             'Sistema de Honor - Peto.
            If UserList(TempCharIndex).flags.Honor > 0 Then
                Stat = Stat & " <Honor: " & UserList(TempCharIndex).flags.Honor & ">"
            End If
...
End If
 ...
 If UserList(TempCharIndex).flags.EsConcilioNegro Then
    ...
    ElseIf FoundChar = 2 Then
            ...
            End If
             If (Npclist(TempCharIndex).POS.Map = Mapa_Castillo And UserList(UserIndex).GuildInfo.GuildName <> "") Then
            Dim Castiact As String
            If Npclist(TempCharIndex).POS.Map = Mapa_Castillo Then Castiact = Castillo
