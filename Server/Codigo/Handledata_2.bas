    ...
    Case "#P"
        ...
        If UserList(UserIndex).flags.Privilegios > 0 Then
            ...
            If Len(tStr) > 0 Then
                ...
                Call SendData(ToIndex, UserIndex, 0, "||" & NumUsers & FONTTYPE_INFO)
                Call SendData(ToIndex, UserIndex, 0, "4L" & NumUsers)
               Call SendData(ToIndex, UserIndex, 0, "|| Record de usuarios: " & recordusuarios & FONTTYPE_INFO)
            Else
...
Call SendData(ToIndex, UserIndex, 0, "||Estas en eventos" & FONTTYPE_TALK)
Exit Sub
End If
...
Else
Call SendData(ToIndex, UserIndex, 0, "||Necesitas 500.000 monedas de oro para viajar." & FONTTYPE_INFO)
Exit Sub
End If
Call WarpUserChar(UserIndex, 166, 17, 55, True)
Call SendData(ToIndex, UserIndex, 0, "||Haz viajado a Castillo de Clanes" & FONTTYPE_TALK)
Exit Sub
...
Exit Sub
End If
Call WarpUserChar(UserIndex, 200, 49, 69, True)
...
Exit Sub
   ...
   Dim NOLOX As Integer
   Dim IL As Integer
   Dim NOLO As Integer
   Dim AUX As Integer
   Dim AUXi As String
   
 UserList(UserIndex).flags.Tdeath = UserList(UserIndex).Faccion.Matados(0) + UserList(UserIndex).Faccion.Matados(1) + UserList(UserIndex).Faccion.Matados(2)
   
   '''''''''''''''''ORDENAMOS EL VECTOR''''''''''''''''''''''
   For IL = 1 To LastUser - 1
   For NOLOX = 2 To LastUser
   If UserList(IL).flags.Tdeath < UserList(NOLOX).flags.Tdeath Then
   AUX = UserList(IL).flags.Tdeath
   UserList(IL).flags.Tdeath = UserList(NOLOX).flags.Tdeath
   UserList(NOLOX).flags.Tdeath = AUX
   AUXi = UserList(IL).Name
   UserList(IL).Name = UserList(NOLOX).Name
   UserList(NOLOX).Name = AUXi
   End If
   Next NOLOX
   Next IL
       
   ''''''''''''''''''ENVIAMOS POR CONSOLA''''''''''''''''''''
   For NOLO = 1 To LastUser
   Call SendData(ToIndex, UserIndex, 0, "||%%%%%%%%%%%%%%%%%%%%RANKING DE USERS ONLINE%%%%%%%%%%%%%%%%%%%%" & FONTTYPE_INFO)
   Call SendData(ToIndex, UserIndex, 0, "||Puesto número" & NOLO & ":" & UserList(NOLO).Name & " - " & UserList(Y).flags.Tdeath & FONTTYPE_INFO)
   Call SendData(ToIndex, UserIndex, 0, "||%%%%%%%%%%%%%%%%%%%%RANKING DE USERS ONLINE%%%%%%%%%%%%%%%%%%%%" & FONTTYPE_INFO)
   Next NOLO
