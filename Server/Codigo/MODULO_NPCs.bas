...
MiNPC = Npclist(NpcIndex)
 ...
 Call MuereReyAZUL(639, UserIndex)
 End If
 If UserList(UserIndex).flags.GrupoRojo = True Then
Call MuereReyROJO(639, UserIndex)
End If
Call QuitarNPC(NpcIndex)
Exit Sub
End If
'agus
   If MiNPC.Name = "Rey" Then
  murioelrey = True
   End If
...
If MiNPC.POS.Map = Mapa_Castillo And MiNPC.Name = "Rey del Castillo" And UserList(UserIndex).GuildInfo.GuildName <> "" Then
Castillo = UserList(UserIndex).GuildInfo.GuildName
 
DiaC = Date
HoraC = Time
Call SendData(ToAll, 0, 0, "||El clan " & UserList(UserIndex).GuildInfo.GuildName & " ha conquistado el castillo del rey." & FONTTYPE_FENIX & ENDC)
Call WriteVar(IniPath & "Castillos.ini", "INIT", "Castillo", UserList(UserIndex).GuildInfo.GuildName)
Call WriteVar(IniPath & "Castillos.ini", "INIT", "DiaC", Date)
Call WriteVar(IniPath & "Castillos.ini", "INIT", "HoraC", Time)
Call SendData(ToAll, 0, 0, "TW" & SND_CREACIONCLAN)
End If
 
If MiNPC.Name = "Rey del Castillo" And MiNPC.Stats.MinHP <= 0 Then
...
End Sub
Sub ReSpawnNpcc(MiNPCp As Npc)

If (MiNPCp.flags.Respawn = 0) Then Call CrearNPC(MiNPCp.Numero, MiNPCp.POS.Map, MiNPCp.Orig)
