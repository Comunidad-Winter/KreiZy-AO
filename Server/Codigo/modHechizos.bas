...
If Hechizos(Spell).RemoverParalisis = 1 Then
     ...
     End If
End If
If Not UserList(UserIndex).Stats.MinHP > 0 Then
Call SendData(ToIndex, UserIndex, 0, ("P8"))
 UserList(UserIndex).flags.Paralizado = 0
      UserList(UserIndex).flags.Paralizado = 0
        UserList(UserIndex).flags.QuienParalizo = 0
End If
End Sub
...
If UserList(UserIndex).POS.Map = 190 Or UserList(UserIndex).POS.Map = 198 Or UserList(UserIndex).POS.Map = 199 Or UserList(UserIndex).POS.Map = 192 Or UserList(UserIndex).POS.Map = 194 Or UserList(UserIndex).POS.Map = 195 Or UserList(UserIndex).POS.Map = 197 Or UserList(UserIndex).POS.Map = 196 Or UserList(UserIndex).POS.Map = 200 Then
...
If UserList(UserIndex).flags.GrupoAzul = True And UserList(tempChr).flags.GrupoAzul = True Then
Call SendData(ToIndex, UserIndex, 0, "||No puedes atacar a un usurio de tu mismo grupo de batalla!" & FONTTYPE_TALK)
Exit Sub
End If
If UserList(UserIndex).flags.GrupoRojo = True And UserList(tempChr).flags.GrupoRojo = True Then
Call SendData(ToIndex, UserIndex, 0, "||No puedes atacar a un usurio de tu mismo grupo de batalla!" & FONTTYPE_TALK)
Exit Sub
End If
...
ElseIf Hechizos(H).SubeHP = 2 Then
    ...
    Else
        ...
        If UserList(UserIndex).flags.Honor < UserList(tempChr).flags.Honor Then
        UserList(UserIndex).flags.Honor = UserList(UserIndex).flags.Honor + 20
   Call SendData(ToIndex, UserIndex, 0, "||Ganastes 20 puntos de honor al matar a un usuario superior a ti!" & FONTTYPE_TALK)
   Call SendData(ToIndex, tempChr, 0, "||Perdistes 10 puntos de honor al morir!" & FONTTYPE_FENIX)
  Else
        UserList(UserIndex).flags.Honor = UserList(UserIndex).flags.Honor + 10
   Call SendData(ToIndex, UserIndex, 0, "||Ganastes 10 puntos de honor al matar a un usuario menor a ti!" & FONTTYPE_TALK)
    Call SendData(ToIndex, tempChr, 0, "||Perdistes 10 puntos de honor al morir!" & FONTTYPE_FENIX)
   End If
