...
Dim LoopC As Integer
    For LoopC = 1 To LastUser
        Castillo = GetVar(IniPath & "Castillos.ini", "INIT", "Castillo")
            If UserList(LoopC).GuildInfo.GuildName = Castillo Then
                UserList(LoopC).flags.Canje = UserList(LoopC).flags.Canje + 1
                Call SendData(ToIndex, (LoopC), 0, "||Has ganado 1 punto de canjeo por tener conquistado el castillo." & FONTTYPE_FENIX)
                Call SendUserStatsBox(LoopC)
                Call SendData(ToIndex, 0, 0, "TW")
        End If
    Next LoopC
Exit Sub
handler:
...
Castillo = GetVar(IniPath & "Castillos.ini", "INIT", "Castillo")
HoraC = GetVar(IniPath & "Castillos.ini", "INIT", "HoraC")
DiaC = GetVar(IniPath & "Castillos.ini", "INIT", "DiaC")
...
Npclist(NpcIndex).GiveEXP = val(GetVar(npcfile, "NPC" & NPCNumber, "GiveEXP")) * 5000
...
Npclist(NpcIndex).GiveGLD = val(GetVar(npcfile, "NPC" & NPCNumber, "GiveGLD")) * 3500
